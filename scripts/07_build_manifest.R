...
field <- function(property, label, format, suffix = NULL) {
...
section_context_2026 <- tooltip(
  ...
  field("population_density_km2", "Residents / km²", "integer"),
  field("under18_pct", "Under 18", "percent"),
  field("age65plus_pct", "65 and older", "percent"),
  field("foreign_citizenship_pct", "Foreign citizenship", "percent")
)
...
layers_out <- list(
  ...
  layer(
    ...
    "Share of residents born outside Spain. This annual census measure has a separate reference date.", "%",
    ...
    tooltip(
      field("foreign_born_pct", "Foreign-born", "percent"),
      field("section_id", "Section ID", "text")
    )
  ),
  layer(
    "building-age", "buildings", "fill", "Earliest construction year", "Construction year",
    "Earliest year in Catastro’s recorded construction interval. The full interval is shown in details.", "year",
...
for (election in names(election_definitions)) {
  ...
  layers_out <- append(layers_out, list(
    ...
    layer(
      ...
      paste(spec$label, "leading party"), "Leading party",
      ...
      control = list(election = election, party = "leading")
    )
  ))
  for (party_key in names(spec$parties)) {
    ...
    layers_out <- append(layers_out, list(
      layer(
        ...
        blue_palette, c(0, 5, 10, 20, 35, 50, 75), "percent",
...
for (mode in names(transport_style)) {
  ...
  layers_out <- append(layers_out, list(layer(
    ...
    paste(style$label, "stops"), "Stops appear only at useful zoom levels.", "stop",
